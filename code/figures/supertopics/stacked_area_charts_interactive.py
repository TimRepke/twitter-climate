...
for st in SuperTopic:
    ...
    # st_topic_counts.append(t_counts.shape[0])
...
fig = make_subplots(rows=7, cols=1, subplot_titles=subplot_titles)
for i, st in enumerate(sts_plot, start=1):
    ...
    for ti, (y_,yt) in enumerate(zip(subfig_y, n_st_tweets.T)):
        fig.add_trace(go.Scatter(x=td_groups,
                                 ...
                                 mode='lines',
                                 stackgroup='one',
                                 name=f'Topic {topic_nums[ti]} ({int(yt.sum()):,} tweets)'), row=i, col=1)

fig.update_layout(height=2000, width=1000)
fig.write_html('data/climate2/figures/supertopic_shares_split.html')
...
fig.update_layout(height=2000, width=1000)
fig.write_html('data/climate2/figures/supertopic_abs_split.html')
